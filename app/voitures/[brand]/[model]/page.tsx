...
export async function generateMetadata({
  ...
  const title = model.metaTitle || `${brand.name} ${model.name} d'occasion en Tunisie | Voito`;
  const description = model.metaDesc || `Achetez et vendez des ${brand.name} ${model.name} d'occasion en Tunisie sur Voito.`;
  ...
  return {
    ...
    alternates: { canonical: `${baseUrl}/voitures/${params.brand}/${params.model}` },
    openGraph: {
      title,
      description,
...
export default async function VoituresBrandModelPage({
  params,
  searchParams,
}: {
  params: { brand: string; model: string };
